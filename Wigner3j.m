...
%     REMARKS:
%     most of the estimates below is based on extensive numerical tests within a range of the quantum numbers <=1000
...
%     (occurring before switching to the symbolic regime);
...
%     and absolute inaccuracy of 0.0001 for Wigner3j(82.5,67,90.5,4.5,1,-5.5,2)=0.0045102 (exact 0.0046133=0.0046133);
...
%   general check, preprocessing
    ...
    NKs=[]; % for pointing to erroneous numeric results in order to switch them to the symbolic computations
%%
...
%   post-processing
    ...
    if ~isempty(NKs) % incorrect numerical results - switch to the symbolic computations
        NKs=sort(NKs);
