function  parts = dipole_T_FF_auto(eL, eR, qa, C, params, opt)
% dipole_T_FF_auto
% ------------------------------------------------------------
% 兼容旧接口的包装函数：
%   内部使用
%       build_D_from_pol_auto
%       build_W_from_detune
%       build_T_from_D
%
% 输入:
...
%              .use_detuning = true/false
%              .force_hermitian = true/false
%
% 输出:
%   T      : 8x8 二阶有效算符
%   parts  : 调试信息
%
% 说明:
%   1) 当 use_detuning = true 时：
%          T = D_L * W * D_R'
%   2) 当 use_detuning = false 时：
%          T = D_L * I * D_R'
%      即只保留偏振/CG 结构，不含失谐因子
% ------------------------------------------------------------
    ...
    % ---------- defaults ----------
    if ~isfield(params, 'delta1')
        params.delta1 = 0;
    end
    if ~isfield(params, 'delta2')
        params.delta2 = params.delta1;
    end

    if ~isfield(opt, 'rule') || isempty(opt.rule)
        opt.rule = 'avg';
    end
    if ~isfield(opt, 'use_detuning')
        opt.use_detuning = true;
    end
    if ~isfield(opt, 'force_hermitian')
        opt.force_hermitian = false;
    end

    % 兼容别名
    if isfield(opt, 'return_only_structure') && opt.return_only_structure
        opt.use_detuning = false;
    end
    if isfield(opt, 'strip_detuning') && opt.strip_detuning
        opt.use_detuning = false;
    end
...
%     T1_on_F2_1 = build_T_from_D(D2, D2, W1_F2_1);
    % ---------- main AC ----------
    dHg_minus = (abs(Om1)^2/4) * (Pa * (T1_on_F1+T2_on_F1)* Pa);
    dHg_plus  = (abs(Om2)^2/4) * (Pb * (T1_on_F2+T2_on_F2) * Pb);

    % ---------- mutual AC ----------
    H_mutual = zeros(N);
%     if params.include_mutual_ac
%         H_mutual = H_mutual ...
%             + (abs(Om1)^2/4) * (Pb * (T22_F1+ T2_on_F2)* Pb) ...
%             + (abs(Om2)^2/4) * (Pa * (T11_F2+T1_on_F1) * Pa);
%     end

    % ---------- Raman ----------
    H12 = (conj(Om1)*Om2/4) * (Pb * T12 * Pa) ;
    Hraman = H12 + H12';

    dHg_0 = H_mutual + Hraman;

    % ---------- parts ----------
    parts.M1           = Pa * (T1_on_F1+ T2_on_F1) * Pa;
    parts.M2           = Pb * (T1_on_F2+T2_on_F2) * Pb;
    parts.Mutual_1onF2 = Pb * T2_on_F2* Pb;
    parts.Mutual_2onF1 = Pa * T2_on_F1 * Pa;
    parts.M12          = Pb * T12 * Pa;


    parts.H_ac_diag = dHg_minus + dHg_plus + H_mutual;
    parts.H_raman   = Hraman;
    parts.H_total   = dHg_minus + dHg_plus + dHg_0;
